...
def xml_to_json(xml_doc):
    ...
    json_doc = xml_node_to_json(doc_el)
    if doc_el.nodeName == 'list' and len(json_doc.keys()) == 1:
        json_doc = json_doc[json_doc.keys()[0]]
...
class Dispatcher(webapp.RequestHandler):
    ...
    def update_impl(self, path, model_name, model_key, method_name, is_replace):
        ...
        resp_type = self.get_query_param(QUERY_TYPE_PARAM) or QUERY_TYPE_FULL
    ...
    def input_to_xml(self):
        ...
        if True or ((content_type != None) and content_type.startswith(JSON_CONTENT_TYPE)):
            j = simplejson.load(self.request.body_file)
            arr = self.request.url.split('/')
            if len(arr) > 0:
                j = {arr[4]: j}
            import cStringIO
            return json_to_xml(cStringIO.StringIO(simplejson.dumps(j)))
